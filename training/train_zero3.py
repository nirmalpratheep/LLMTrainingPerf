"""FSDP Zero3 training loop with optimizations."""
...
def train_zero3(
    ...
    """
    Train with FSDP Zero3 strategy.
    ...
    print(f"[Rank {rank}] Starting Zero3 training...")
    ...
    # Wrap with FSDP Zero3
    fsdp_config = get_fsdp_config(strategy="zero3", dtype=torch.bfloat16)
    model = FSDP(model, **fsdp_config)
    
    if rank == 0:
        print(f"Model wrapped with FSDP Zero3")
    ...
    if use_profiler and rank == 0:
        prof = profile(
            ...
            on_trace_ready=lambda p: p.export_chrome_trace(f"{output_dir}/trace_zero3.json"),
            record_shapes=True,
            profile_memory=True,
            with_stack=True,
        )
    ...
    for step in range(num_steps):
        ...
        # Backward pass (Zero3 has more communication here)
...
def main():
    parser = argparse.ArgumentParser(description="FSDP Zero3 Training")
